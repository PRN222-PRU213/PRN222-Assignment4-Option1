...
public interface IExchangeRateRepository
{
    ...
    Task<int> GetCountAsync(DateTime? startDate, DateTime? endDate, string? baseCurrency, string? targetCurrencies, CancellationToken ct = default);
    Task<List<WorkerLog>> GetWorkerLogsAsync(int count, CancellationToken ct = default);
    Task<bool> HasDataForDateAsync(DateTime date, CancellationToken ct = default);
}
